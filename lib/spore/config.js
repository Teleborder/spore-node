...
Config.prototype.appEnv = getSet('appEnv');
...
Config.prototype.netrc = function () {
  if(!this._netrc) {
    this._netrc = netrc;
    this._netrc.filename = resolvePath(this.get('netrc'));
  }
...
Config.prototype.load = function () {
  ...
  if(process.env[this.envPrefix + 'ENV']) {
    debug("Overriding default environment with " + this.envPrefix + 'ENV');
    this.defaultEnv(process.env[this.envPrefix + 'ENV']);
  }
